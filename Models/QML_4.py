...
N_CLASSES  = 3
N_FEATURES = 8
EPOCHS     = 5      # set low for testing; increase to 30-40 for final results
BATCH_SIZE = 32
LR         = 0.008
...
# ═══════════════════════════════════════════════════════════════════════════════
#  2.  DATASET  —  Kim & Choi (2022/2023) 5G Traffic Dataset  [REAL CSV FILES]
# ═══════════════════════════════════════════════════════════════════════════════
"""
CSV FORMAT (Wireshark export — identical across all files)
──────────────────────────────────────────────────────────
  Columns: No. | Time | Source | Destination | Protocol | Length | Info

  No.         : Wireshark packet number (integer, not used)
  Time        : absolute timestamp  "2022-09-27 13:08:31.564846"
  Source      : sender IP address   e.g. "10.215.173.1"
  Destination : receiver IP address e.g. "112.217.128.200"
  Protocol    : TCP / UDP / DNS / TLS / QUIC …
  Length      : IP packet length in bytes (integer)
  Info        : Wireshark summary  e.g. "58632  >  443 [SYN] …"

DEVICE IP HEURISTIC
───────────────────
  The phone (Samsung Galaxy A90 5G) always appears as a private IP:
    10.x.x.x  or  192.x.x.x  or  172.16-31.x.x
  Packets where Source = private IP  → UPLINK   (phone → server)
  Packets where Source = public  IP  → DOWNLINK (server → phone)
  DL/UL ratio = downlink_bytes / uplink_bytes

PORT PARSING (from Info field)
───────────────────────────────
  Info column contains strings like "58632  >  443 [SYN]"
  We extract dst_port = second number after ">".
  Well-known ports (<1024) on destination → uplink (client request to server)

SLICE MAPPING (ITU IMT-2020 / 3GPP TS 22.261)
───────────────────────────────────────────────
  eMBB  (0): Stored_Streaming + Live_Streaming + Video_Conferencing
  URLLC (1): Game_Streaming + Online_Game
  mMTC  (2): Metaverse
"""
...
# ── Top-level folder → slice label ───────────────────────────────────────────
FOLDER_SLICE_MAP = {
    'stored_streaming'  : 0,   # eMBB
    'live_streaming'    : 0,   # eMBB
    'video_conferencing': 0,   # eMBB
    'game_streaming'    : 1,   # URLLC
    'online_game'       : 1,   # URLLC
    'metaverse'         : 2,   # mMTC
}
...
# ── Private IP detector ───────────────────────────────────────────────────────
def _is_private(ip):
    """Return True if ip string is an RFC-1918 / loopback / link-local address."""
    try:
...
# ── Port extractor from Wireshark Info string ─────────────────────────────────
_PORT_RE = _re.compile(r'(\d+)\s*[>→]\s*(\d+)')
def _dst_port(info_str):
    """Parse 'srcport  >  dstport …' from Wireshark Info column. Returns int or None."""
    m = _PORT_RE.search(str(info_str))
    return int(m.group(2)) if m else None

# ─────────────────────────────────────────────────────────────────────────────
#  Single-file loader
# ─────────────────────────────────────────────────────────────────────────────
def _load_one_csv(path, label, max_windows=200):
    """
    Parse one Wireshark CSV and extract 1-second sliding-window features.

    Column layout guaranteed by dataset:
      No. | Time | Source | Destination | Protocol | Length | Info

    Returns DataFrame with FEAT_COLS + ['traffic_class'], or empty DataFrame.
    """
    try:
    ...
    # ── Direction: uplink = phone→server, downlink = server→phone ─────────────
    df_raw['_uplink'] = df_raw['Source'].apply(_is_private)

    # ── Extract dst port from Info for secondary DL/UL check ──────────────────
    if 'Info' in df_raw.columns:
        df_raw['_dport'] = df_raw['Info'].apply(_dst_port)
    else:
        df_raw['_dport'] = None
    ...
    while t + 1.0 <= t_end and len(rows) < max_windows:
        ...
        times_w = win['_ts'].values.astype(np.float64)
        iats_us = np.diff(times_w) * 1e6      # seconds → microseconds

        # DL/UL ratio using source-IP direction flag
        up_bytes   = win.loc[ win['_uplink'], '_len'].sum()
        down_bytes = win.loc[~win['_uplink'], '_len'].sum()
        if up_bytes > 0:
            dl_ul = float(down_bytes) / float(up_bytes)
        else:
            dl_ul = float(down_bytes + 1.0)   # all downlink
    ...
    roll = df_feat['throughput_kbps'].rolling(5, min_periods=1)
    df_feat['bitrate_mean_kbps'] = roll.mean()
    df_feat['bitrate_std_kbps']  = roll.std().fillna(0.0)
    df_feat['traffic_class']     = label

    # Hard-clip physically impossible values
    df_feat['throughput_kbps']       = df_feat['throughput_kbps'].clip(lower=1.0)
    df_feat['avg_pkt_size_bytes']    = df_feat['avg_pkt_size_bytes'].clip(lower=20.0, upper=1500.0)
    df_feat['inter_arrival_mean_us'] = df_feat['inter_arrival_mean_us'].clip(lower=0.01)
    df_feat['inter_arrival_std_us']  = df_feat['inter_arrival_std_us'].clip(lower=0.0)
    df_feat['flow_pkt_count']        = df_feat['flow_pkt_count'].clip(lower=1.0)
    df_feat['dl_ul_ratio']           = df_feat['dl_ul_ratio'].clip(lower=0.01, upper=1000.0)
    df_feat['bitrate_mean_kbps']     = df_feat['bitrate_mean_kbps'].clip(lower=0.1)
    df_feat['bitrate_std_kbps']      = df_feat['bitrate_std_kbps'].clip(lower=0.0)

    return df_feat[feat_cols + ['traffic_class']]

# ─────────────────────────────────────────────────────────────────────────────
#  Full dataset loader — walks folder tree
# ─────────────────────────────────────────────────────────────────────────────
...
def balance_and_cap(df, max_per_class=300):
    ...
    if n_each < 30:
        raise ValueError(
            f"Smallest class has only {n_each} windows after loading.\n"
            f"  Class counts: {counts.to_dict()}\n"
            f"  Check that DATASET_PATH is correct and CSVs are readable."
        )
...
# ── Synthetic fallback (used only if real load fails) ─────────────────────────
def _synth_fallback(n=900):
    rng = np.random.default_rng(42); rows=[]; labels=[]
    spc = n // N_CLASSES
    params = [
        # eMBB:  throughput, pkt_size, iat_mean, iat_std, pkt_cnt, dl_ul, br_mean, br_std
...
print( "      Format : Wireshark packet-capture CSV  "
       "(No. | Time | Source | Destination | Protocol | Length | Info)")
print()
